...
int a,b;
	...
	wskaznik_pliku=fopen(nazwapliku_model,"r"); 
...
int zrob_drzewo(struct symbol_zrodla model_zrodla[], struct galaz drzewo_kodowania[],int liczba_symboli_modelu, char *nazwapliku_drzewo,char *nazwapliku_mmod)
{
...
unsigned char bajt, bajtek[4], jedynka=1;
    ...
    FILE *wskaznikpliku_tkodow=NULL;
	wskaznikpliku_tkodow=fopen(nazwa_tkodow,"wb"); 
...
//       printf("%d %d \n", model_zrodla[n].symbol, model_zrodla[n].czestosc);
    ...
    n=nazwa_pliku(nazwa_pliku_in, rozszerzenie_mmod,nazwa_modmod);
    
    liczba_symboli_dodanych=zrob_drzewo(model_zrodla,drzewo_kodowania,liczba_symboli_modelu,nazwapliku_drzewo,nazwa_modmod);
    ...
    if (liczba_symboli_dodanych!=0)
    {
     ...
     printf("Zmodyfikowany model zrodla zapisany do pliku %s\n", nazwa_modmod);
     }
    else
    {
